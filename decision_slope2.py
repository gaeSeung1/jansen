...
def set_path3(image, forward_criteria):
    ...
    try:
        ...
        center_y2 = (np.ones(forward_check_number)*2*center-left_line+right_line)/2-center
        center_x2 = np.vstack((np.arange(forward_check_number), np.zeros(forward_check_number)))   

        m2, c2 = np.linalg.lstsq(center_x2.T, center_y2, rcond=-1)[0]
        if abs(m2) < 0.35:
            short_forward = 'yes'
        else:
            short_forward = 'no'
...
while True:
    ...
    elif path_number == 1: # 구간 1-2 사이에 있음
        result, slope, forward, short_forward = set_path3(image, 5)
        break
